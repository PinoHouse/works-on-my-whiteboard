...
设计转码流水线，是把一个不可变媒体源经过探测、解码、切片、多个编码档、音轨/字幕处理、质检和封装，最终发布成一组可播放产物。成功不是某个 worker 返回 0，而是当前 DAG revision 的所有必需节点都有可验证产物，质量策略通过，并由一个 release manifest 原子暴露。队列重试、推测执行和 worker 迟到都不能让旧结果污染新发布。
...
实体为 `SourceGeneration`、`PipelineDefinition(revision)`、`Job(job_id,input_digest,revision)`、`TaskNode(dependencies,resource_class)`、`Attempt(attempt_no,fence_token,lease)`、`Artifact(digest,producer_attempt)` 和 `ReleaseManifest`。DAG revision 冻结节点、边、参数、必需/可选标志和处理器镜像。节点只有所有当前父 artifact 成功且摘要可读时进入 runnable；调度器发 attempt 租约与单调 fencing token。

attempt 输出先写不可变临时 artifact，再向任务状态机提交 token、输入父摘要与输出摘要。只有当前 attempt token 可 CAS 任务为 succeeded；旧 attempt 的字节可清理但不能成为下游输入。Job 在必需叶子完成后进入 VERIFYING，校验时间轴、音视频同步、关键帧和 manifest 引用，最后原子提交 release。下游 key 包含源 digest、DAG revision、父 artifact digest 和处理器版本，可安全复用真正相同结果。
...
[DEDUCED:transcoding-pipeline-release-must-isolate-partial-completion] 五个 rendition 完成四个时直接更新标题目录，播放器可能选到仍不存在的第五档或在清单刷新中看到不同集合。各 task 成功不等于可播放集合成功；release manifest 必须在必需项完整、兼容与质检通过后一次发布。若产品允许渐进发布，每个阶段也必须是独立完整 release generation，而非可变半成品。
...
提交 Job 时冻结源 digest 与 DAG revision。协调器持久化任务状态，依赖满足才发带 token 的租约；worker 续租、写 attempt 专属不可变 artifact、校验后提交。失败分为确定性输入/配方错误、可重试基础设施错误和资源不足，只有后两类退避重试；取消新 revision 会提升 job generation，使旧 worker 提交被拒。release verifier 从权威任务状态读取摘要，写 manifest 临时对象并原子提交。
...
观察 job 交付时延、各 stage queue age/运行时、关键路径、GPU/CPU 利用、租约超时、fence 拒绝、重试浪费、artifact cache 命中、部分完成年龄、质检失败和发布原子性。待校准两组门槛：队列年龄/预计剩余时间达到 deadline 多少比例触发扩容或拒绝；推测执行在运行时哪个分位启动，节省尾延迟是否大于双算成本。

故障时间线：0 分钟 job J revision 8；10 分钟 encode attempt 1 租约丢失；11 分钟 attempt 2 token 22 开始；15 分钟 2 成功并使下游 runnable；16 分钟 1 token 21 迟交，被 fence，不能覆盖。随后四个必需叶子中一个缺失，release 保持不可见；补齐、质检通过才提交 manifest R8。complete 响应丢失时，以 release id 查询返回同一 R8。
...
本题本质是：给定“只有当前 DAG revision 的完整、经验证依赖闭包才能发布”，因为 worker 是否仍在运行、长尾何时结束和异构资源何时可用都不可立即获知，候选人应推导依赖状态机、租约加 fencing、不可变 artifact 与原子 release，并在并行、复用、deadline 和重复计算间选择。

优秀回答会画 DAG 而不是组件列表，算关键路径与 `H×Σri`，构造 attempt 1 迟交覆盖 attempt 2 的反例，并区分 task success 与 release success。常见误区是把消息到达当依赖、任意重试覆盖同一路径、用队列长度代表工作量、少一档也直接改清单，或只谈扩 GPU 不做 admission。
