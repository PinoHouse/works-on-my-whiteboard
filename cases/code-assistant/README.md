...
第三步支持跨文件 patch 与 PR，但应用仍走 expected_head CAS 和分支保护。若每个被接受建议的验证 GPU/CPU 成本超过人工节省，缩小候选数或只对高风险变更跑完整测试；若安全误报造成接受率下降，则按语言与路径校准，而不关闭 secret 扫描。
...
给定“助手只能基于获授权的确定仓库状态提出候选，且候选不得未经当前树验证成为提交”这一不变量，因为仓库远大于上下文、revision 与权限不断变化、模型又不具备提交权威，候选人应推导出 revision/permission-bound context、结构化预算选择、expected-head CAS 和分层验证，并在质量、交互延迟、验证成本与泄露风险间决定。
...
二十分钟给出 context manifest、patch 状态机、`R≫B` 预算和 stale-head 反例；四十分钟加入符号图、权限缓存、沙箱验证与过载降级；六十分钟讨论多语言工具链、灰度评测、撤权清除、提示注入和成本归因。追问用“head 100 生成、101 时应用”检验写入边界，用“撤权后旧 embedding 命中”检验隐私，用“定义与测试都不在打开文件”检验上下文选择。
