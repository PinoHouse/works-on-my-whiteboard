...
本设计以 payment intent 为业务 owner，intent 幂等键固定金额、币种、商户和订单；每个逻辑 capture 再由 `capture_id` 和 capture 幂等键固定金额与目标授权。外部 attempt 先关联 capture 落本地 outbox 与稳定 reference，再由 worker 串行发送；同步响应、回调、主动查询和批次对账先按 reference 找 attempt，再收敛所属 capture 的单调状态。重复同一 capture 的请求返回既有 C1，参数变化直接拒绝；不同 C2 即便金额相同也按授权剩余额度独立处理。响应丢失不会创建新逻辑 capture 或并发 attempt；若渠道合同允许后续 attempt，它仍属于原 C1，且任一外部成功后封住其余未发送 attempt。
...
故障演练：10:00:00 逻辑 capture C1 的 attempt A1/reference P7 已持久化并发出，10:00:01 处理器成功，10:00:02 服务故障丢响应，10:00:05 客户端以 C1 和原幂等键重试。系统返回 C1 的 CAPTURE_PENDING 并查询 P7，不能创建同金额 C2；10:00:08 回调与查询结果并发到达，只允许 C1 一次转为 CAPTURED 并写一组账本分录。随后商户以新 `capture_id=C2` 合法捕获剩余的同样 100 元时，应建立独立 capture，而迟到的 C1 timeout 不得影响 C2 或降级 C1。

状态机升级先重放历史事件做影子比较，灰度少量商户；回滚继续解析新状态和 reference。密钥轮换保留旧回调验签窗口，敏感字段令牌化，按商户限制数据访问。新增渠道前用故障注入验证超时、重复回调、乱序和批次差异，而不仅验证成功路径。

## 面试考察本质

这题考察的是：给定“同一逻辑 capture 不重复扣款、合法 partial capture 不被误去重且成功语义诚实”的不变量，因为本地超时无法知道外部处理器是否已经捕获，候选人应推导出 intent、`capture_id/idempotency_key`、一对多外部 attempt、授权/捕获/结算分层和参考号对账，并依据重复扣款损失与等待体验决定何时查询、降级或切渠道。

优秀回答会把 UNKNOWN 画成一等状态，说明谁拥有外部事实，让 capture 幂等键固定 `capture_id` 与请求参数，区分“重试 C1”和“同金额新建 C2”，并用响应丢失时间线解释为何重试不等于恢复。常见误区是宣称消息队列提供端到端恰好一次、把授权当到账、在未知时新建 capture、按金额误杀 partial capture，或把对账当可延期报表。
